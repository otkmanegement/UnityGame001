...
public class MagicChant : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(gameManager.GetComponent<PlayButton>().GetPlayFlug()==1){
            if(this.GetComponent<CharaSet>().charaNum==1){
                count += 1;
                targets = GameObject.FindGameObjectsWithTag("Enemy");
    ...
    void Attack(){
        Instantiate(fire,GetWorldPositionFromRectPosition(rect),Quaternion.identity,canvas.transform);
